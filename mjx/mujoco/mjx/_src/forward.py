...
def fwd_actuation(m: Model, d: Data) -> Data:
    ...
    d = d.replace(act_dot=act_dot, qfrc_actuator=qfrc_actuator, actuator_force=force)
    return d
...
@named_scope
def discrete_lagrangian(m: Model, d: Data, q1: jax.Array, q2: jax.Array):
    """Discretize lagrangian"""
    d = d.replace(qpos=(q1 + q2) / 2, qvel=(q2 - q1) / m.opt.timestep)
    # TODO(): validate that those parts from `forward` are enough
    d = smooth.kinematics(m, d)
    d = sensor.energy_pos(m, d)
    d = sensor.energy_vel(m, d)
    return (d.energy[1] - d.energy[0]) * m.opt.timestep


@named_scope
def discrete_forces(m: Model, d: Data, q1: jax.Array, q2: jax.Array):
    """Discretize the applied forces"""
    d = d.replace(qpos=(q1 + q2) / 2, qvel=(q2 - q1) / m.opt.timestep)
    # TODO(): Whole forward is not required, call only necessary methods.
    d = forward(m, d)
    return 0.5 * d.qfrc_applied * m.opt.timestep, 0.5 * d.qfrc_applied * m.opt.timestep


@named_scope
def vi_stabilize(carry):
    """"""
    m, d, p1, q2 = carry
    A = jax.jacobian(jax.grad(discrete_lagrangian, 2), 3)(m, d, d.qpos, q2)
    F_l, _ = discrete_forces(m, d, d.qpos, q2)
    p1_hat = -jax.grad(discrete_lagrangian, 2)(m, d, d.qpos, q2) - F_l
    gradient = jp.linalg.inv(A) @ (p1 - p1_hat)
    return m, d, p1, q2 - gradient


@named_scope
def variational(m: Model, d: Data) -> Data:
    """Integrates using DELM"""
    if not m.opt.enableflags & EnableBit.ENERGY:
        return d
    p1 = support.mul_m(m, d, d.qvel)
    q2_guess = d.qpos + m.opt.timestep * d.qvel

    # TODO(): Write stabilization until convergence
    n_steps = 2
    for i in range(n_steps):
        _, _, _, q2_guess = vi_stabilize((m, d, p1, q2_guess))

    _, F_r = discrete_forces(m, d, d.qpos, q2_guess)
    p2 = jax.grad(discrete_lagrangian, 3, allow_int=True)(m, d, d.qpos, q2_guess) + F_r
    v2 = smooth.solve_m(m, d, p2)

    # TODO(): Check whether this is enough/proper way to progress simulation.
    time = d.time + m.opt.timestep
    act = _next_activation(m, d, d.act_dot)
    d = d.replace(qpos=q2_guess, qvel=v2, act=act, time=time)
    return d
...
def step(m: Model, d: Data) -> Data:
    ...
    if m.opt.integrator == IntegratorType.EULER:
        ...
        d = implicit(m, d)
    elif m.opt.integrator == IntegratorType.VARIATIONAL:
        d = variational(m, d)
    else:
