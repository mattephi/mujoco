...
def sensor_pos(m: Model, d: Data) -> Data:
    ...
    for sensor_type in set(m.sensor_type[stage_pos]):
        ...
        if sensor_type == SensorType.MAGNETOMETER:
            sensor = jax.vmap(lambda xmat: xmat.T @ m.opt.magnetic)(d.site_xmat[objid])
    ...
    sensordata = d.sensordata.at[np.concatenate(adrs)].set(jp.concatenate(sensors))
...
def sensor_acc(m: Model, d: Data) -> Data:
    ...
    for sensor_type in sensor_types:
        ...
        if sensor_type == SensorType.TOUCH:
            ...
            conray = jax.vmap(lambda frame, force: math.normalize(frame[0] * force[0]))(
            ...
            site_xmat = d.site_xmat[objid]
            sensor = jax.vmap(lambda mat, vec: mat.T @ vec)(site_xmat, cfrc_int[:, 3:])
            adr = (adr[:, None] + np.arange(3)[None]).reshape(-1)
...
def energy_pos(m: Model, d: Data) -> Data:
    ...
    #     print("Combination", not m.opt.disableflags & DisableBit.PASSIVE)
    #     print("Only passive:", DisableBit.PASSIVE)
    #     print("Only disable flags", not m.opt.disableflags)
    #     print("tendon length size ", m.tendon_lengthspring.size)
    #     print("tendon length inequality", m.tendon_lengthspring.size > 0)
    #     print(
    #         "Together ",
    #         (not m.opt.disableflags & DisableBit.PASSIVE)
    #         & (m.tendon_lengthspring.size > 0),
    #     )
